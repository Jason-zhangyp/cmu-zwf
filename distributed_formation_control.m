...
function [poses_out] = distributed_formation_control(poses_in, formation, dt)
    ...
    %% homogenize the histogram
    for i=1:N
        if histogram(i)<=1
            continue
        end
        %% move extra elements toward dense direction
        if i>1&&histogram(i-1)==0
           comp_vec = formation(:,i)-formation(:,i-1);
           for j=1:histogram(i)
               %???如何从histogram对应到aim当中去
           end
        end
