...
function [poses_out, assignment] = perfect_formation_control5(poses_in, formation, dt, count, assignment)
    ...
    if mod(count,floor(N^0.5/dt))==1
        ...
        while rest_num>=1
            ...
            distance_test = zeros(N,2);
            for i=1:N %agents
                distance_i = ones(1,N)*999;
                if agent_check(i)==1
                    continue
                end
                for j=1:N %targets
                    if target_check(j)==0%||count == 1131
                        distance_i(j) = sqrt((formation(j,1)-poses_in(i,1)+pose_x-mean_x)^2+(formation(j,2)-poses_in(i,2)+pose_y-mean_y)^2);
                        %distance(i) = distance(i) + sqrt((formation(j,1)-poses_in(i,1)+pose_x-mean_x)^2+(formation(j,2)-poses_in(i,2)+pose_y-mean_y)^2);
                    end
                end
                [distance_test(i,2),distance_test(i,1)] = min(distance_i);
            end
            [max_t,max_i]=max(distance_test(:,2));%subscript of the farthest agent
            if max_t < 1 && rest_num==N
                break
            end
            agent_check(max_i) = 1;
            target_check(distance_test(max_i,1)) = 1;
            assignment(max_i) = distance_test(max_i,1);
